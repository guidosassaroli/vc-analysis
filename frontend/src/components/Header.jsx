...
export default function Header({ onRefresh, onScoreAll, onCleared }) {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      showToast('Failed to refresh feed. Check your connection.', 'error')
    } finally {
      setRefreshing(false)
    }
  }
  ...
  const handleScoreAll = async () => {
    ...
    try {
      ...
      showToast('Scoring failed. Check your ANTHROPIC_API_KEY.', 'error')
    } finally {
      setScoring(false)
    }
  }
  ...
  const handleClearAll = async () => {
    if (!window.confirm('Clear all startups and reset to seed data? This cannot be undone.')) return
    setClearing(true)
    try {
  ...
  return (
    <>
        ...
        <div className="max-w-screen-2xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={clearing}
              className="btn bg-white/5 hover:bg-red-600/80 text-white/60 hover:text-white border border-white/10 hover:border-red-500 disabled:opacity-50 transition-colors"
              title="Clear all startups and reset to seed data"
            >
              {clearing ? (
            ...
            >
              ...
              <span className="hidden lg:inline text-white/40 text-xs">HN · GitHub · News</span>
            </button>
      ...
      {/* Toast notification */}
      {toast && (
        <div className={`fixed bottom-6 right-6 z-50 animate-slide-up flex items-start gap-3 px-5 py-4 rounded-xl shadow-xl max-w-sm text-sm font-medium ${
          toast.type === 'error'
